...
void lihatRiwayatStruk() {
    ...
    for (int i = 0; i < jumlahStruk; i++) {
        printf("\n========================================\n");
        printf("Struk ke-%d\n", i + 1);
        printf("Tanggal: %s | Waktu: %s\n", riwayatStruk[i].tanggal, riwayatStruk[i].waktu);
        printf("----------------------------------------\n");
        printf("Pesanan        Qty       Harga\n");
        printf("----------------------------------------\n");

        for (int j = 0; j < riwayatStruk[i].jumlahBarang; j++) {
            printf("%-14s %3d     Rp %6d\n", riwayatStruk[i].barang[j], riwayatStruk[i].qty[j], riwayatStruk[i].harga[j]);
        }

        printf("----------------------------------------\n");
        printf("Subtotal:                Rp %6.2f\n", riwayatStruk[i].total);
        printf("Diskon (%.0f%%):        -Rp %6.2f\n", riwayatStruk[i].total * (riwayatStruk[i].diskon / 100));
        printf("Subtotal Setelah Diskon: Rp %6.2f\n", riwayatStruk[i].total - riwayatStruk[i].total * (riwayatStruk[i].diskon / 100));
        printf("Pajak (10%%):            Rp %6.2f\n", riwayatStruk[i].pajak);
        printf("Total:                   Rp %6.2f\n", riwayatStruk[i].totalAkhir);
        printf("Uang pembayaran:         Rp %6.2f\n", riwayatStruk[i].uang);
        printf("Kembalian:               Rp %6.2f\n", riwayatStruk[i].kembali);
        printf("========================================\n");
    }
}
